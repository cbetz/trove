...
- **`sdoh`** — Social Determinants of Health enrichments. v0.1 ships county-level Area Deprivation Index aggregation from UW's Neighborhood Atlas block-group release. Local-pipeline use only — UW's terms are non-sublicensable, so ADI-derived columns aren't included in the public bundles. Run the gap pipeline locally with your own UW download to attach service-area deprivation to your local `data/parquet/community_benefit_gap_2022.parquet`. Public-surface SDOH (likely CDC SVI) is on the v2 roadmap.
...
- **M5.1** — `sdoh` package added with a `county_adi_from_block_group()` aggregator over UW's Neighborhood Atlas block-group ADI release. ADI flows through the *local* pipeline (gitignored Parquet + local CSV) for any user who downloads the raw block-group data themselves under their own UW license. ADI is **not** included in the public bundles or the public CSV artifact: UW's terms are non-sublicensable, so derived ADI columns can't be redistributed via troveproject.com. Public-surface SDOH (likely CDC SVI) is on the v2 roadmap.
