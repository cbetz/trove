# Community Benefit Cross-Form Dataset — TY2022 (HCRIS FY2023 × IRS 990 Schedule H TY2022)
...
- **1,295 are computable** — both Schedule H 7a (financial assistance at cost) and HCRIS S-10 (charity care cost) are present so a cross-form difference can be calculated. The other 39 have a blank 990 7a and are matched but not compared.
...
For every nonprofit hospital system whose EIN appears in both the CBI crosswalk and the IRS 990 Schedule H feed for tax year 2022, we sum the per-facility HCRIS Worksheet S-10 charity care cost across all CCNs that share an EIN, then place it beside that system's Schedule H Part I line 7a "financial assistance at cost" — a related, non-identical figure each form reports for the same broad underlying activity.

`charity_gap = HCRIS S-10 charity care cost − Schedule H 7a financial assistance at cost`

Positive value → the HCRIS S-10 amount is higher than the Schedule H line 7a amount.

## Headline numbers

**Aligned + material subset (228 systems — close-period view):**

- HCRIS and 990 fiscal periods overlap within 1 month, and both filings ≥ $500K
- Median absolute proportional difference: **25.1%**
- **53 systems (23%)** differ by more than 50% on this cross-form measure
- **$1.08B** total absolute difference on this subset against **$14.21B** Schedule H community benefit reported (a 7.6% reconciliation delta)
...
- The "$5B absolute difference on the full set" figure that earlier versions of this dataset published is structurally misleading because most of those dollars come from misaligned-period views. The aligned-subset $1.08B / 7.6%-of-CB figure is the defensible version.

## Distribution of differences on the aligned + material subset (228 systems)

This is a lookup tool. The per-row data is in the CSV, but this summary reports distribution rather than curated hospital rankings.

**Absolute dollar difference (`|charity_gap|`):**

| P25 | P50 (median) | P75 | P90 | Max |
|-----|--------------|-----|-----|-----|
| $0.3M | $1.2M | $4.3M | $12.4M | $77.5M |

**Proportional difference (`|charity_gap| / max(HCRIS, 990)`):**
...
A positive value (HCRIS > 990) does not by itself say whether either filing is wrong. Plausible reasons the same hospital has different numbers on each form:
...
This is a **home-county proxy**, not a true catchment-area measure. Hospital service areas typically extend beyond the home county, and within-county variation is invisible at this resolution. Do not use it to infer hospital service-area need or adequacy of charity care.
...
- **No reconciliation pass:** Hospitals with EINs not yet in CBI's crosswalk aren't matched at all. A self-built CCN↔EIN reconciliation against IRS BMF would close this unmatched segment.
